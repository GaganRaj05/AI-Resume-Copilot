...
@celery_app.task(
    ...
    max_retries=3,
    dont_autoretry_for=(FileNotFoundError, ValueError, TypeError),
)
def process_document(
    self, user_id: str, doc_id: str, file_path: str, original_filename: str
) -> dict:
    logger.info("[%s] Task started | file=%s", doc_id, file_path)

    def _update(step: str, pct: int):
        self.update_state(state="PROGRESS", meta={"step": step, "percent": pct})
